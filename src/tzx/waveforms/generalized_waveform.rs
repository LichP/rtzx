...
// Iterates pulses for use by a [GeneralizedWaveform].
#[derive(Clone, Debug)]
...
/// A waveform for generalized data encoding as used by [GeneralizedDataBlock](crate::tzx::blocks::GeneralizedDataBlock).
///
/// Data is encoded as a sequence of symbols, each symbol comprised of a sequence of pulses. The data payload is chunked to the
/// width of the symbol key size (e.g. if there are seven symbols in the symbol definition table, the key width is three bits),
/// and each symbol key is used to look up to a definition in the symbols table.
///
/// For simplicity of on-disk encoding, each symbols definition contains a fixed size list of pulse lengths equal to the size of
/// the longest such list defined for any symbol. Symbols using fewer pulses encoded unused pulses as having length zero, so
/// these entries are ignored.
///
/// The pulse sequence is generated by [GeneralizedPulseIterator], and the waveform itself iterates the samples.
#[derive(Debug)]
...
impl GeneralizedWaveform {
    // Constructs a generalized waveform.
    pub fn new(config: Arc<Config>, symbols: Arc<Vec<SymbolDefinition>>, payload: DataPayload, start_pulse_high: bool) -> Self {
    ...
    /// Indicates whether the last pulse in the waveform is high or low.
